...
public class JWCSecurityFilter implements Filter {

	private static final Logger LOGGER = Logger.getLogger(JWCSecurityFilter.class);
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
		...
		if (request.getRequestURI().equals("/produto/admin")) {
			...
			if (token == null) {
				...
				try {
					...
					if (request.getRequestURI().equals("/produto/admin")) {
						chain.doFilter(req, res);
						String admin = TokenParser.parse(token, "admin");
						validateAdmin(admin);
	...
	private void validateAdmin(String admin) {
		if (!loggedInUsers.contains(admin)) {
			throw new RuntimeException("admin invalido");
		}
	}
